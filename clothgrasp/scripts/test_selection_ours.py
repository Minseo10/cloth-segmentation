...
from selectgrasp_ours import GraspSelector
...
class ClothGrasper:
    def __init__(self):
        ...
        self.grasp_target = param['grasp_target']
    ...
    def _aggregate_data(self, rgb_im, depth_im, prediction, oe, ie, cor, py, px, angle, in_py, in_px, var, ang_x, ang_y):
        corners = None
        outer_edges = None
        inner_edges = None
        inner_py = None
        ...
        if self.detection_method == 'groundtruth':
            ...
            if len(angle_x) != 0:
                ...
                variance = np.reshape(var, shape)
                angle_x = np.reshape(ang_x, shape)
                angle_y = np.reshape(ang_y, shape)
    ...
    def _save_train(self, data, plot, count):
        ...
        cv2.imwrite(os.path.join(dir_path, fname % (count, "red")), red)
        cv2.imwrite(os.path.join(dir_path, fname % (count, "yellow")), yellow)
        cv2.imwrite(os.path.join(dir_path, fname % (count, "green")), green)
        ...
        fname = "%s_%d.png"
        cv2.imwrite(os.path.join(dir_path, fname % ("rgb", count)), rgb)
        fname = "%d_%s.npy"
        np.save(os.path.join(dir_path, fname % (count, "depth")), depth)

        angle_x = data['angle_x']
        angle_y = data['angle_y']
        fname = "%d_direction_%s.npy"
        np.save(os.path.join(dir_path, fname % (count, "x")), angle_x)
        np.save(os.path.join(dir_path, fname % (count, "y")), angle_y)

        var = data['variance']
        fname = "%d_variance.npy"
        np.save(os.path.join(dir_path, fname % count), var)

        invvar = 1 / (var + 0.0001)
        fname = "%d_invvariance.npy"
        np.save(os.path.join(dir_path, fname % count), invvar)
    ...
    def run(self):
        ...
        py, px, angle, inner_py, inner_px, var, angle_x, angle_y = g._server_cb(rgb_im, depth_im, prediction, outer_edges, inner_edges, corners)
        data = self._aggregate_data(rgb_im, depth_im, prediction, outer_edges, inner_edges, corners, py, px, angle, inner_py, inner_px, var, angle_x, angle_y)
        plot = self.visualizer.visualize(data, show_grasp=True)
        return data, plot
