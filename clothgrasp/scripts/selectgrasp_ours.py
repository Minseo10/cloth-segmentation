...
class NetworkGraspSelector:
    ...
    def select_grasp(self, rgb, corners, outer_edges, inner_edges, pred, retries=1, num_neighbour=8):
        ...
        if self.grasp_point_method == 'policy':
            ...
            if self.grasp_point_method == 'manual':
                ...
                _, lbl = cv2.distanceTransformWithLabels(inner_edges_filt.astype(np.uint8), cv2.DIST_L2, 5,
                ...
                print("Number of min var indices: %d\n" % len(min_idxs))
        ...
        if self.grasp_angle_method == 'inneredge':
            temp, lbl = cv2.distanceTransformWithLabels(inner_edges.astype(np.uint8), cv2.DIST_L2, 5,
                                                        labelType=cv2.DIST_LABEL_PIXEL)
    ...
    def plot(impred, xx_o, yy_o, var, outer_edges_filt, xx, yy, segmentation):
        ...
        ax.quiver(xx_o[::factor], yy_o[::factor], direction_o[::factor, 1] - xx_o[::factor],
                  -direction_o[::factor, 0] + yy_o[::factor], color='white', scale=1, scale_units='x')
...
class GraspSelector():
    ...
    def _init_selector(self):
        if self.detection_method == 'groundtruth':
            ...
            grasp_point_method = param['grasp_point_method']
    ...
    def _server_cb(self, rgb, depth_im, prediction, outer_edges, inner_edges, corners):
        ...
        if self.detection_method == 'groundtruth':
            ...
            outer_edges = deepcopy(outer_edges)
            inner_edges = deepcopy(inner_edges)
            rgb = deepcopy(rgb)
            pred = deepcopy(prediction)
            py, px, angle, inner_py, inner_px = self.selector.select_grasp(rgb, corners, outer_edges, inner_edges, pred)
        else:
            pred = deepcopy(prediction)
            py, px, angle = self.selector.select_grasp(pred)
            print(py, px, angle)

        ## fix
        print('Sending grasp selection response py: %f, px: %f, angle: %f\n' % (py, px, angle))
        # response = SelectGraspResponse()

        if inner_py != None and inner_px != None:
            print('inner_py: %f, inner_px: %f\n' % (inner_py, inner_px))
        if self.detection_method == 'groundtruth' and px != 0:
            var_ = var.flatten()
            angle_x_ = angle_x.flatten()
            angle_y_ = angle_y.flatten()

        return py, px, angle, inner_py, inner_px, var_, angle_x_, angle_y_
#
#     def run(self):
#          # rospy.spin()
#
